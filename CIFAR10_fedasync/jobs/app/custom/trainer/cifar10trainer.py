...
class Cifar10Trainer(Executor):
    def __init__(
        ...
        self._train_dataset = CIFAR10(root=data_path, transform=transforms_train, download=True, train=True)

        self.best_acc = {}
        self.loss_value = {}

        self.traindata, self.n_iteration = self.trainloader()
    ...
    def trainloader(self):
        with open('/ailab/user/gushuang/experiment/asyn/CIFAR10_fedasync/train_6clients_balance_iid.pkl', 'rb') as f:
            loaded_data = torch.load(f)

        # Dictionary to store data loaders and the number of iterations for each client
        client_loaders = {}
        iterations_dict = {}

        # Create a train DataLoader for each client 
        for i, client_indices in loaded_data.items():
            client_dataset = Subset(self._train_dataset, client_indices)
            client_loader = DataLoader(client_dataset, batch_size=128, shuffle=True)
            client_loaders[f'site-{i+1}'] = client_loader

            num_iterations = len(client_loader)
            iterations_dict[f'site-{i+1}'] = num_iterations

            # Initialize the best accuracy for the current client
            self.best_acc[f'site-{i+1}'] = 0
            self.loss_value[f'site-{i+1}'] = []

        return client_loaders, iterations_dict

    def testloader(self):
        with open('/ailab/user/gushuang/experiment/asyn/CIFAR10_fedasync/test_6clients_balance_iid.pkl', 'rb') as f:
            loaded_data = torch.load(f)

        # Dictionary to store data loaders for each client
        client_loaders = {}

        # Create a test DataLoader for each client 
        for i, client_indices in loaded_data.items():
    ...
    def _get_model_weights(self,fl_ctx) -> Shareable:
        ...
        train_site = fl_ctx.get_identity_name()
        n_iterations = self.n_iteration[train_site]
    ...
    def _local_train(self, fl_ctx: FLContext, weights, abort_signal):
        # Set the model weights
        self.model.load_state_dict(state_dict=weights)

        # Get the current training site and corresponding data loaders
        train_site = fl_ctx.get_identity_name()
        train_loader = self.traindata[train_site]

        #before training, calculate the global model's test accuracy on corresponding site
        best_acc = self.best_acc.get(train_site)
        test_loader = self.testdata[train_site]
        acc_initial = self._validate(test_loader,abort_signal)
        self.log_info(
                   fl_ctx,  f"Test Acc: {acc_initial*100}%"
                )
        if acc_initial>best_acc:
            best_acc = acc_initial
            self.best_acc[train_site] = best_acc
            self._save_local_model(fl_ctx)
        filepath = "testacc"
        if not os.path.exists(filepath):
            with open(filepath, 'w') as file:
                file.write("epoch test_accuracy\n")
        with open(filepath, 'a') as file:
            file.write(f"{acc_initial}\n")
        ...
        for epoch in range(self._epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if (i+1) % 10 == 0:
                    self.loss_value[train_site].append(running_loss)
                    acc = self._validate(test_loader,abort_signal)
                    self.log_info(
                        fl_ctx, f"Epoch: {epoch}/{self._epochs}, Iteration: {i}, " f"Loss: {running_loss/10} " f"Acc: {acc*100}%"
                    )
    ...
    def _save_local_model(self, fl_ctx: FLContext):
        ...
        floder = "app_" + site_name
        run_dir = fl_ctx.get_engine().get_workspace().get_run_dir(fl_ctx.get_prop(ReservedKey.RUN_NUM))
        models_dir = os.path.join(run_dir, floder)
        models_dir = os.path.join(models_dir, PTConstants.PTModelsDir)
        if not os.path.exists(models_dir):
            os.makedirs(models_dir)
        model_path = os.path.join(models_dir, PTConstants.PTLocalModelName)
        self.log_info(fl_ctx,f"save {site_name}'s best local model in: {model_path}")
    ...
    def _load_local_model(self, fl_ctx: FLContext):
        ...
        with open('losses.txt', 'w') as f:
            for loss in self.loss_value[site_name]:
                f.write(f"{loss}\n")
    ...
    def _validate(self, test_loader, abort_signal):
        self.model.eval()
        correct = 0
        total = 0
        with torch.no_grad():
